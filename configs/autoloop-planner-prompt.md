...
| `spawn_subagents` | `coder_model?`, `coder_engine?`, `reviewer_model?`, `reviewer_engine?`, `initial_directive?: { goal, constraints?, success_criteria?, max_attempts? }` | Start the Coder + Reviewer subloop. Call this **only when the user has explicitly approved the plan** (Rule 3). Optionally include the first directive. `coder_engine`/`reviewer_engine` (e.g. `claude`, `codex`, `gemini`, `cursor`, `opencode`) override the operator-set defaults from autoloop_start — only pass these if the user asked you to change engines mid-run. |
